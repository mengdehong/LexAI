...
function extractJsonPayload(raw: string): string {
    ...
    if (trimmed.startsWith("```") && trimmed.endsWith("```")) {
        const inner = trimmed.replace(/^```[a-zA-Z]*\s*/, "").replace(/```$/, "").trim();
        return extractJsonPayload(inner); // Recursive call after removing code block
    }
    
    // Strategy: Find valid JSON by parsing incrementally
    // This handles cases where LLM adds text after the JSON
    
    // Try array first (most common for term extraction)
    ...
    if (arrayStart !== -1) {
        const substring = trimmed.slice(arrayStart);
        let depth = 0;
        let inString = false;
        let escape = false;
...
async function resolveApiKey(provider: ProviderConfig): Promise<string> {
    console.log(`[resolveApiKey] Resolving key for provider: id=${provider.id}, vendor=${provider.vendor}, name=${provider.name}`);

    // 1. Try exact provider ID match (most specific)
    try {
        const stored = await getApiKey(provider.id);
        if (stored) {
            console.log(`[resolveApiKey] ✓ Found key via provider.id '${provider.id}'`);
            return stored;
        }
    } catch (error) {
        console.error(`[resolveApiKey] Error checking provider.id '${provider.id}':`, error);
    }

    // 2. Try vendor fallback (for backward compatibility)
    try {
        const byVendor = await getApiKey(provider.vendor);
        if (byVendor) {
            console.log(`[resolveApiKey] ✓ Found key via vendor '${provider.vendor}'`);
            return byVendor;
        }

        // Special case: "gemini" vendor might be stored as "google"
        if (provider.vendor === "gemini") {
            const byGoogle = await getApiKey("google");
            if (byGoogle) {
                console.log(`[resolveApiKey] ✓ Found key via alias 'google'`);
                return byGoogle;
            }
        }
    } catch (error) {
        console.error(`[resolveApiKey] Error checking vendor '${provider.vendor}':`, error);
    }

    // 3. Try provider name as last Tauri fallback
    try {
        const byName = await getApiKey(provider.name);
        if (byName) {
            console.log(`[resolveApiKey] ✓ Found key via provider.name '${provider.name}'`);
            return byName;
        }
    } catch (error) {
        console.error(`[resolveApiKey] Error checking provider.name '${provider.name}':`, error);
    }

    // 4. Try environment variables (development fallback)
    ...
    for (const key of candidates) {
        ...
        if (value && value.trim().length > 0) {
            console.log(`[resolveApiKey] ✓ Found key via env var '${key}'`);
            return value.trim();
        }
    }

    console.warn(`[resolveApiKey] ✗ No API key found for provider '${provider.id}' (vendor: ${provider.vendor})`);
    return "";
}
...
export async function testProvider(provider: ProviderConfig, keyOverride?: string): Promise<void> {
    ...
    console.log(`[testProvider] Testing provider: id=${provider.id}, vendor=${vendor}, keyOverride=${keyOverride ? 'YES' : 'NO'}`);

    // Determine which API key to use
    let apiKey = (keyOverride ?? "").trim();

    if (!apiKey) {
        console.log(`[testProvider] No keyOverride, attempting to resolve stored key...`);
        apiKey = await resolveApiKey(provider);
    }

    if (!apiKey) {
        const hint = `Missing API key for provider "${provider.name}" (id: ${provider.id}). ` +
            `Please save the API key in Settings first.`;
        console.error(`[testProvider] ${hint}`);
        throw new Error(hint);
    }

    console.log(`[testProvider] Using API key (length: ${apiKey.length})`);
    ...
    if (vendor === "gemini") {
        try {
            ...
            console.log(`[testProvider] ✓ Gemini provider test successful`);
            return;
        } catch (e) {
    ...
    try {
        ...
        console.log(`[testProvider] ✓ OpenAI-compatible provider test successful`);
    } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        throw new Error(
...
function parseTermDefinitions(jsonString: string): TermDefinition[] {
    ...
    try {
        ...
        const preview = jsonString.length > 500 ? jsonString.slice(0, 500) + '...' : jsonString;
        console.error('[parseTermDefinitions] JSON parse failed. Preview:', preview);
        throw new Error(`Failed to parse LLM response as JSON. The model may have returned invalid JSON. Error: ${err instanceof Error ? err.message : String(err)}`);
    }
    
    if (!Array.isArray(parsed)) {
        throw new Error('Expected JSON array from LLM, but received: ' + typeof parsed);
    }
