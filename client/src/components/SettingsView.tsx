...
export function SettingsView({ onLanguageChange }: SettingsViewProps = {}) {
    ...
    const handleProviderSubmit = useCallback(
        async (event: FormEvent<HTMLFormElement>) => {
            ...
            try {
                ...
                if (!providerForm.id) {
                    // New provider: auto-assign to unconfigured operations
                    ...
                    // Editing existing provider: update all mappings using this provider
                    // with the new defaultModel
                    const nextMapping: ModelMapping = { ...modelMapping };
                    const newDrafts = { ...modelDrafts };

                    MAPPING_OPERATIONS.forEach((operation) => {
                        const mapping = nextMapping[operation];
                        if (mapping && mapping.providerId === identifier) {
                            mapping.model = entry.defaultModel;
                            newDrafts[operation] = entry.defaultModel;
                ...
                if (providerForm.id) {
                    if (updatedMappings) {
                        infoMessage = isChinese
                            ? "Provider 已更新，所有相关功能的模型已同步更新。"
                            : "Provider updated. All associated function models have been synced.";
                    } else {
    ...
    return (
        <div className="settings-grid">
            {/* Browser mode warning - commented out
            {!isTauriRuntime && (
                <div style={{
                    gridColumn: '1 / -1',
                    padding: '12px 16px',
                    ...
                    ⚠️ {isChinese
                        ? '当前在浏览器中运行，API 密钥保存功能不可用。请使用 Tauri 桌面应用窗口。'
                        : 'Running in browser mode. API key storage is unavailable. Please use the Tauri desktop application window.'}
                </div>
            )}
            */}
            <section className="panel">
